...
function BottomNav({ active, onNavigate, unread = 0 }) {
  // Publish the nav's REAL height. The player bar used a hardcoded 64px, which
  // is wrong once the safe-area inset is added — hence the floating gap.
  ...
  return (
    <nav ref={navRef} className="letters-bottomnav" style={{
    ...
    <article onClick={() => onOpen && onOpen(item)}
      ...
      <div style={{ display:"flex", gap:12, minWidth:0 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {/* Full post text — short, so no clamp; preserve line breaks and linkify #tags */}
          <p
            style={{ margin:0, fontSize:15.5, lineHeight:1.6, color:"#222", fontFamily:"'EB Garamond', Georgia, serif", whiteSpace:"pre-wrap" }}
            onClick={(e) => { const a = e.target.closest && e.target.closest("[data-topic]"); if (a) { e.stopPropagation(); e.preventDefault(); navigate(`/topic/${a.getAttribute("data-topic")}`); } }}
            dangerouslySetInnerHTML={{ __html: linkifyTags(item.fullBody || item.preview || "") }}
          />
...
function FeedPage({ onSignOut, session, onNavigate, activeTab }) {
  // Keyboard height, straight from the visual viewport. Needed INLINE below:
  // the reader overlay's geometry is written inline by React, and inline styles
  // beat stylesheet rules — a .kb-open CSS rule silently loses to them.
  const [kbHeight, setKbHeight] = useState(0);
  useEffect(() => {
    const vv = window.visualViewport;
    if (!vv) return;
    const onResize = () => setKbHeight(Math.max(0, Math.round(window.innerHeight - vv.height)));
    onResize();
  ...
  const setOpenLetter = (item) => {
    ...
    // Powers "Lately you've read" on the Write desk. Mock seed letters have no
    // uuid, so only real ones are recorded.
    if (item.dbId) supabase.rpc("mark_read", { p_kind: "letter", p_ref: item.dbId }).then(() => {}, () => {});
        ...
        >
          {openLetter ? (
            <div style={{ width:"100%", paddingBottom:60 }}>
              {/* Close bar */}
              <div style={{ position:"sticky", top:0, background:"rgba(253,252,248,0.97)", backdropFilter:"blur(10px)", borderBottom:"1px solid #F0EDE8", padding:"0 24px", paddingTop:"env(safe-area-inset-top, 0px)", height:"calc(48px + env(safe-area-inset-top, 0px))", boxSizing:"border-box", display:"flex", alignItems:"center", justifyContent:"space-between", zIndex:10 }}>
                <span style={{ fontSize:10, letterSpacing:"0.14em", textTransform:"uppercase", color:"#AAA", fontFamily:"'DM Mono', monospace" }}>Letter</span>
                <button onClick={() => setOpenLetter(null)} style={{ background:"none", border:"none", cursor:"pointer", color:"#8A8170", padding:"8px 4px", display:"flex", alignItems:"center", gap:5, fontSize:12, fontFamily:"'DM Sans', sans-serif" }}>
                  Close
...
function ReadPage({ onNavigate, session }) {
  ...
  const setOpenArticle = (article) => {
    ...
    if (article.dbId) supabase.rpc("mark_read", { p_kind: "article", p_ref: article.dbId }).then(() => {}, () => {});
...
// ── The Writing Desk ────────────────────────────────────────────────────────
// The Write tab opens here, not in a composer. Reading comes first at Letters,
// so the desk shows what you've read and offers to quote it; publications and
// drafts are the work in progress. The composer is one click away, and always
// knows where the writing is going.
//
// Sections are ALWAYS rendered, empty or not — the scaffolding teaches the
// shape of the product on day one (you'll have publications; drafts persist;
// what you read becomes what you write about) and the layout never jumps as
// they fill.

const WRITING_QUOTES = [
  { q: "Good prose is like a windowpane.", by: "George Orwell" },
  { q: "I write entirely to find out what I'm thinking.", by: "Joan Didion" },
  { q: "Writing is thinking on paper.", by: "William Zinsser" },
  { q: "Omit needless words.", by: "Strunk & White" },
  { q: "Write the book you want to read.", by: "Austin Kleon" },
  { q: "Easy reading is damn hard writing.", by: "Nathaniel Hawthorne" },
  { q: "The first draft of anything is trash.", by: "Attributed to Hemingway" },
  { q: "Vigorous writing is concise.", by: "William Strunk Jr." },
];
// Same quote all day, different each day — steady, not slot-machine.
function quoteOfTheDay() {
  const days = Math.floor(Date.now() / 86400000);
  return WRITING_QUOTES[days % WRITING_QUOTES.length];
}

function WriteDeskPage({ session, onNavigate }) {
  const navigate = useNavigate();
  const uid = session?.user?.id;
  ...
  const load = async () => {
    ...
    const [{ data: p }, { data: d }, { data: h }] = await Promise.all([
      supabase.from("publications").select("*").eq("owner_id", uid).eq("archived", false).order("created_at"),
      supabase.from("drafts").select("*, publication:publication_id (title)").eq("user_id", uid).order("updated_at", { ascending: false }).limit(4),
      supabase.from("reading_history").select("*").eq("user_id", uid).order("read_at", { ascending: false }).limit(6),
    ]);
    setPubs(p || []);
    setDrafts(d || []);

    // Hydrate the reads: one query per kind, then merge back in read_at order.
    const hist = h || [];
    if (hist.length) {
      const ids = (k) => hist.filter(x => x.kind === k).map(x => x.ref_id);
      const [arts, lets, eps] = await Promise.all([
        ids("article").length ? supabase.from("news_articles").select("id, title, source").in("id", ids("article")) : { data: [] },
        ids("letter").length  ? supabase.from("letters").select("id, title, body, profiles:user_id (full_name)").in("id", ids("letter")) : { data: [] },
        ids("episode").length ? supabase.from("podcast_episodes").select("id, title, show:show_id (title)").in("id", ids("episode")) : { data: [] },
      ]);
      const byId = {};
      (arts.data || []).forEach(a => byId["article:" + a.id] = { kind: "article", id: a.id, title: a.title, src: a.source });
      (lets.data || []).forEach(l => byId["letter:" + l.id]  = { kind: "letter", id: l.id, title: l.title || (l.body || "").replace(/<[^>]*>/g, " ").slice(0, 70), src: "A letter by " + (l.profiles?.full_name || "someone") });
      (eps.data  || []).forEach(e => byId["episode:" + e.id] = { kind: "episode", id: e.id, title: e.title, src: e.show?.title });
      setReads(hist.map(x => byId[x.kind + ":" + x.ref_id]).filter(Boolean));
    } else setReads([]);
    setLoading(false);
  };
  useEffect(() => { load(); }, [uid]);

  // Every route into the composer carries its destination.
  const compose = (state) => navigate("/write/compose", { state: state || {} });

  const S = {
    slot: { border: "1px dashed #DED7C7", borderRadius: 3, padding: "20px", textAlign: "center", background: "#FDFCF9" },
    slotP: { fontFamily: "'EB Garamond', Georgia, serif", fontSize: 15, lineHeight: 1.6, color: "#A79E8C", maxWidth: 300, margin: "0 auto" },
    secHead: { display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 12 },
    h2: { fontFamily: "'DM Mono', monospace", fontSize: 10, letterSpacing: "0.18em", textTransform: "uppercase", color: "#B0A488", fontWeight: 400 },
    link: { fontFamily: "'DM Mono', monospace", fontSize: 10.5, color: "#C8A96E", background: "none", border: "none", cursor: "pointer" },
  };

  const kindMark = { article: "\u275D", letter: "\u00A7", episode: "\u266A" };
  const kindCta  = { article: "Quote it \u2192", letter: "Reply at length \u2192", episode: "Clip a passage \u2192" };

  return (
    <div className="letters-main" style={{ minHeight: "100vh", background: "#F9F6F0", paddingBottom: 90 }}>
      <TopBar title={<span>Write<span style={{ color: "#C8A96E" }}>.</span></span>} maxWidth={1040} onLogoClick={() => onNavigate && onNavigate("feed")}/>

      <main style={{ maxWidth: 920, margin: "0 auto", padding: "34px 22px 0" }}>
        {/* Head: dateline + a line about the craft */}
        <div style={{ textAlign: "center" }}>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 46, fontWeight: 900, letterSpacing: "-0.03em", lineHeight: 1, margin: 0 }}>
            Write<span style={{ color: "#C8A96E" }}>.</span>
          </h1>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, letterSpacing: "0.22em", textTransform: "uppercase", color: "#B0A488", marginTop: 12 }}>{today}</div>
          <div style={{ maxWidth: 430, margin: "22px auto 24px" }}>
            <p style={{ fontFamily: "'EB Garamond', Georgia, serif", fontStyle: "italic", fontSize: 19, lineHeight: 1.5, color: "#3A3328", margin: 0 }}>&ldquo;{quote.q}&rdquo;</p>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B0A488", marginTop: 9 }}>{quote.by}</div>
          </div>
        </div>

        <hr style={{ border: 0, borderTop: "1px solid #E8E0D0", margin: 0 }}/>

        {/* The door — sits on the rule */}
        <div style={{ textAlign: "center", marginTop: -23, marginBottom: 34, position: "relative", zIndex: 5 }}>
          <button onClick={() => setPicker(true)}
            style={{ background: "#141414", color: "#F0EAD8", border: "none", borderRadius: 26, padding: "13px 38px", fontFamily: "'DM Sans', sans-serif", fontSize: 15, fontWeight: 600, cursor: "pointer", boxShadow: "0 6px 22px rgba(20,18,14,0.18)", display: "inline-flex", alignItems: "center", gap: 9 }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z"/></svg>
            Write
          </button>
        </div>

        <div className="desk-cols" style={{ display: "grid", gridTemplateColumns: "1.35fr 1fr", gap: 32 }}>
          {/* LEFT — what you're making */}
          <div>
            <div style={{ marginBottom: 28 }}>
              <div style={S.secHead}>
                <h2 style={S.h2}>Your publications</h2>
                {pubs.length > 0 && <button style={S.link} onClick={() => setNewPub(true)}>+ New</button>}
              </div>
              {pubs.length === 0 ? (
                <div style={S.slot}>
                  <div style={{ borderTop: "1px solid #E4DDCD", borderBottom: "1px solid #E4DDCD", padding: "7px 0 5px", margin: "0 auto 11px", maxWidth: 190 }}>
                    <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, fontWeight: 900, color: "#DDD5C4", letterSpacing: "-0.02em" }}>Your title here</span>
                  </div>
                  <p style={S.slotP}>
                    Your publications go here — <em style={{ color: "#B8AF9C" }}>newsletters, columns, blogs, poems.</em> Each one gets its own name, its own readers, and its own archive.
                  </p>
                  <button onClick={() => setNewPub(true)}
                    style={{ marginTop: 12, background: "none", border: "1px solid #E8E0D0", borderRadius: 18, padding: "6px 15px", fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 600, color: "#8A8170", cursor: "pointer" }}>
                    Start a publication
                  </button>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {pubs.map(p => (
                    <div key={p.id} onClick={() => compose({ publication: p })}
                      style={{ background: "#fff", border: "1px solid #E8E0D0", borderRadius: 3, padding: "15px 16px 13px", cursor: "pointer", textAlign: "center" }}>
                      <div style={{ borderTop: "1px solid #141414", borderBottom: "1px solid #141414", padding: "8px 0 6px", marginBottom: 9 }}>
                        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 21, fontWeight: 900, letterSpacing: "-0.02em" }}>
                          {p.title}<span style={{ color: "#C8A96E" }}>.</span>
                        </div>
                        {p.standfirst && <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, letterSpacing: "0.18em", textTransform: "uppercase", color: "#B0A488", marginTop: 6 }}>{p.standfirst}</div>}
                      </div>
                      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, color: "#8A8170" }}>
                        {CADENCE_LABEL[p.cadence] || "When it's ready"} · <b style={{ color: "#8A6D2F", fontWeight: 500 }}>Issue {p.issue_count + 1} next</b>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div style={{ marginBottom: 28 }}>
              <div style={S.secHead}>
                <h2 style={S.h2}>Drafts</h2>
              </div>
              {drafts.length === 0 ? (
                <div style={{ ...S.slot, padding: "16px 20px" }}>
                  <p style={{ ...S.slotP, fontSize: 14.5 }}>
                    Your saved drafts go here. Nothing you write is lost — <em style={{ color: "#B8AF9C" }}>close the page and come back to it.</em>
                  </p>
                </div>
              ) : (
                <div style={{ background: "#FBF8F1", border: "1px solid #E8E0D0", borderRadius: 3, padding: "3px 16px" }}>
                  {drafts.map(d => {
                    const words = (d.body || "").replace(/<[^>]*>/g, " ").trim().split(/\s+/).filter(Boolean).length;
                    return (
                      <div key={d.id} onClick={() => compose({ draft: d })}
                        style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "11px 0", borderBottom: "1px solid #F0EDE8", cursor: "pointer" }}>
                        <div>
                        ...
                        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 8.5, padding: "2px 7px", borderRadius: 10, whiteSpace: "nowrap", flexShrink: 0,
                          color: d.publication ? "#8A6D2F" : "#B0A488", background: d.publication ? "#F5EFE2" : "none", border: "1px solid " + (d.publication ? "#E8DFC9" : "#F0EDE8") }}>
                          {d.publication ? d.publication.title : "Blank page"}
                        </span>
                      </div>
                    );
                  })}
          ...
          {/* RIGHT — what you've been reading */}
          <div>
            <div style={S.secHead}><h2 style={S.h2}>Lately you&rsquo;ve read</h2></div>
            {reads.length === 0 ? (
              <div style={{ ...S.slot, textAlign: "left", padding: 16 }}>
                <div style={{ display: "flex", flexDirection: "column", gap: 9, marginBottom: 12 }}>
                  {[0,1,2].map(i => (
                    <div key={i} style={{ display: "flex", gap: 10, alignItems: "center", opacity: 0.55 }}>
                      <div style={{ width: 26, height: 26, border: "1px dashed #DED7C7", borderRadius: 2, flexShrink: 0 }}/>
                      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4 }}>
                        <i style={{ display: "block", height: 6, width: "38%", background: "#EAE3D4", borderRadius: 2 }}/>
                        <i style={{ display: "block", height: 8, width: "82%", background: "#EAE3D4", borderRadius: 2 }}/>
                      </div>
                    </div>
                  ))}
                </div>
                <p style={{ ...S.slotP, maxWidth: "none", textAlign: "left", fontSize: 14.5 }}>
                  What you read — <em style={{ color: "#B8AF9C" }}>articles, podcasts, other people&rsquo;s letters</em> — shows up here, with an offer to quote it.
                </p>
              </div>
            ) : (
              <>
                <p style={{ fontFamily: "'EB Garamond', Georgia, serif", fontStyle: "italic", fontSize: 15.5, color: "#8A8170", margin: "-2px 0 6px" }}>
                  Something to say about one of these?
                </p>
                {reads.map(r => (
                  <button key={r.kind + r.id} onClick={() => compose({ read: r })}
                    className="desk-read"
                    style={{ display: "flex", gap: 11, alignItems: "flex-start", background: "none", border: 0, borderBottom: "1px solid #F0EDE8", padding: "13px 0", cursor: "pointer", textAlign: "left", width: "100%" }}>
                    <div style={{ width: 28, height: 28, border: "1px solid #E8E0D0", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", color: "#8A6D2F", fontSize: 12, flexShrink: 0, background: "#fff" }}>
                      {kindMark[r.kind]}
                    </div>
                    <div>
                      <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8.5, letterSpacing: "0.13em", textTransform: "uppercase", color: "#8A6D2F", marginBottom: 3 }}>{r.src}</div>
                      <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 15.5, fontWeight: 700, lineHeight: 1.3 }}>{r.title}</div>
                      <span className="desk-cta" style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, color: "#C8A96E", display: "block", marginTop: 5 }}>{kindCta[r.kind]}</span>
                    </div>
                  </button>
                ))}
              </>
      ...
      {picker && (
        <WritePicker pubs={pubs}
          onClose={() => setPicker(false)}
          onPick={(dest) => { setPicker(false); if (dest === "new") setNewPub(true); else compose(dest); }}/>
      )}
      {newPub && (
        <NewPublicationModal session={session}
          onClose={() => setNewPub(false)}
          onCreated={(p) => { setNewPub(false); load(); compose({ publication: p }); }}/>
      )}
    </div>
  );
}

const CADENCE_LABEL = { weekly: "Weekly", fortnightly: "Fortnightly", monthly: "Monthly", when_ready: "When it's ready" };

// ── The picker: where does this go? ─────────────────────────────────────────
// With no publications yet it shows two doors and REVERSES the emphasis —
// "blank page" becomes primary, because that's what a new writer should do
// first. Starting a publication is offered, never pushed.
function WritePicker({ pubs, onClose, onPick }) {
  const fresh = pubs.length === 0;
  const Opt = ({ mark, ghost, title, sub, onClick }) => (
    <button onClick={onClick}
      style={{ display: "flex", alignItems: "center", gap: 13, padding: "14px 24px", cursor: "pointer", background: "none", border: 0, width: "100%", textAlign: "left" }}
      onMouseEnter={e => e.currentTarget.style.background = "#FBF8F1"}
      onMouseLeave={e => e.currentTarget.style.background = "none"}>
      <div style={{ width: 34, height: 34, borderRadius: 2, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: 15,
        background: ghost ? "none" : "#141414", color: ghost ? "#B0A488" : "#C8A96E", border: ghost ? "1px dashed #E8E0D0" : "none" }}>{mark}</div>
      <div style={{ flex: 1 }}>
        <div style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 14, fontWeight: 700, marginBottom: 2 }}>{title}</div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, color: "#B0A488", lineHeight: 1.5 }}>{sub}</div>
      </div>
      <span style={{ color: "#D8D0C2", fontSize: 14 }}>&rsaquo;</span>
    </button>
  );

  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,18,14,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2200, padding: 22 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#F9F6F0", borderRadius: 5, width: "100%", maxWidth: 430, overflow: "hidden", boxShadow: "0 30px 80px rgba(0,0,0,0.4)" }}>
        <div style={{ textAlign: "center", padding: "24px 24px 18px", borderBottom: "1px solid #E8E0D0" }}>
          <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 25, fontWeight: 900, letterSpacing: "-0.02em", margin: 0 }}>Where does this go?</h3>
          <p style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, letterSpacing: "0.1em", textTransform: "uppercase", color: "#B0A488", marginTop: 8 }}>You can change it before publishing</p>
        </div>
        ...
        <div style={{ padding: "8px 0" }}>
          {fresh ? (
            ...
            <>
              {pubs.map(p => (
                <Opt key={p.id} mark={p.title.charAt(0).toUpperCase()} title={p.title}
                  sub={`${CADENCE_LABEL[p.cadence]} · issue ${p.issue_count + 1} · ${p.follower_count} ${p.follower_count === 1 ? "reader" : "readers"}`}
        ...
        <div style={{ padding: "12px 24px 18px", textAlign: "center" }}>
          <button onClick={onClose} style={{ background: "none", border: "none", fontFamily: "'DM Mono', monospace", fontSize: 10.5, color: "#B0A488", cursor: "pointer" }}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
// ── Start a publication ─────────────────────────────────────────────────────
function NewPublicationModal({ session, onClose, onCreated }) {
  ...
  const create = async () => {
    ...
    const { data, error } = await supabase.from("publications")
      .insert({ owner_id: session.user.id, title: title.trim(), standfirst: standfirst.trim() || null, description: description.trim() || null, cadence })
      .select().single();
    setBusy(false);
    if (error) { setErr(error.message); return; }
    track("publication_created", { cadence });
    onCreated(data);
  };

  const L = { fontFamily: "'DM Mono', monospace", fontSize: 9.5, letterSpacing: "0.14em", textTransform: "uppercase", color: "#B0A488", display: "block", marginBottom: 6 };
  const I = { width: "100%", border: "1px solid #E8E0D0", borderRadius: 2, padding: "10px 12px", fontFamily: "'EB Garamond', Georgia, serif", fontSize: 16, color: "#141414", outline: "none", background: "#fff", marginBottom: 16 };

  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(20,18,14,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2300, padding: 22 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#F9F6F0", borderRadius: 5, width: "100%", maxWidth: 520, maxHeight: "88vh", overflowY: "auto", padding: "28px 30px", boxShadow: "0 30px 80px rgba(0,0,0,0.4)" }}>
        <div style={{ textAlign: "center", marginBottom: 22 }}>
          <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 900, letterSpacing: "-0.02em", margin: 0 }}>Start a publication</h3>
          <p style={{ fontFamily: "'EB Garamond', Georgia, serif", fontStyle: "italic", fontSize: 16, color: "#8A8170", marginTop: 6 }}>A title of your own, with its own readers.</p>
        </div>

        {/* Live nameplate — you see the thing you're making */}
        <div style={{ background: "#fff", border: "1px solid #E8E0D0", borderRadius: 3, padding: 22, marginBottom: 22 }}>
          <div style={{ borderTop: "1px solid #141414", borderBottom: "1px solid #141414", padding: "10px 0 8px", textAlign: "center" }}>
            <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 900, letterSpacing: "-0.02em", color: title ? "#141414" : "#DDD5C4" }}>
              {title || "Your title"}<span style={{ color: "#C8A96E" }}>.</span>
            </div>
            <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8, letterSpacing: "0.2em", textTransform: "uppercase", color: standfirst ? "#B0A488" : "#DDD5C4", marginTop: 7 }}>
              {standfirst || "A line about what it is"}
            </div>
          </div>
        </div>

        <label style={L}>Title</label>
        <input style={I} value={title} onChange={e => setTitle(e.target.value)} placeholder="The Counter" maxLength={60}/>

        <label style={L}>Standfirst — the line under the nameplate</label>
        <input style={I} value={standfirst} onChange={e => setStandfirst(e.target.value)} placeholder="Dispatches from behind the pharmacy counter" maxLength={90}/>

        <label style={L}>What is it about?</label>
        <input style={I} value={description} onChange={e => setDescription(e.target.value)} placeholder="What working retail pharmacy taught me about American healthcare." maxLength={200}/>

        <label style={L}>How often, roughly?</label>
        <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
          {["weekly", "fortnightly", "monthly", "when_ready"].map(c => (
            <button key={c} onClick={() => setCadence(c)}
              style={{ flex: 1, fontFamily: "'DM Mono', monospace", fontSize: 10.5, border: "1px solid " + (cadence === c ? "#141414" : "#E8E0D0"), background: cadence === c ? "#141414" : "#fff", color: cadence === c ? "#F0EAD8" : "#8A8170", padding: "8px 4px", borderRadius: 2, cursor: "pointer" }}>
              {CADENCE_LABEL[c]}
            </button>
          ))}
        </div>
        <p style={{ fontFamily: "'DM Mono', monospace", fontSize: 9.5, color: "#B0A488", lineHeight: 1.6, marginBottom: 6 }}>
          A promise to readers, not a deadline. Nothing forces you to publish.
        </p>

        {err && <div style={{ color: "#B9756B", fontFamily: "'DM Mono', monospace", fontSize: 11, marginTop: 10 }}>{err}</div>}

        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 22 }}>
          <button onClick={onClose} style={{ background: "none", border: "1px solid #E0D8CC", borderRadius: 22, padding: "9px 18px", fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: "#777", cursor: "pointer" }}>Cancel</button>
          <button onClick={create} disabled={busy}
            style={{ background: "#141414", color: "#F0EAD8", border: "none", borderRadius: 22, padding: "9px 22px", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: busy ? "default" : "pointer", opacity: busy ? 0.6 : 1 }}>
...
function WritePage({ session, onNavigate }) {
  ...
  useEffect(() => {
    ...
    // Destination + prefill, handed over by the Write desk.
    const st = location.state || {};
    if (st.publication) setPublication(st.publication);
    if (st.draft) {
      const d = st.draft;
      setDraftId(d.id);
    ...
    if (st.read) {
      const r = st.read;
      if (r.kind === "article") setForm(f => ({ ...f, sourceTitle: r.title || "", sourcePublication: r.src || "" }));
    }
  ...
  const [hashSource, setHashSource] = useState("editor"); // which field the # menu is driving: 'editor' | 'post'
  ...
  // Post composer (a plain textarea) gets the same # menu as the letter editor.
  // We can't use a DOM Range here, so measure the caret with a mirror div.
  const caretXY = (ta) => {
    const div = document.createElement("div");
    const cs = window.getComputedStyle(ta);
    ["fontFamily","fontSize","fontWeight","lineHeight","letterSpacing","padding","border","boxSizing","whiteSpace","wordWrap","width"].forEach(k => {
      div.style[k] = cs[k];
    });
    ...
    const y = rect.top + (marker.offsetTop - ta.scrollTop) + parseFloat(cs.lineHeight || cs.fontSize);
    document.body.removeChild(div);
    return { x: Math.min(x, window.innerWidth - 316), y: y + 6 };
  };

  const detectPostHashMenu = () => {
    const ta = postRef.current;
    if (!ta) { setHashMenu(null); return; }
    const before = ta.value.slice(0, ta.selectionStart);
    const m = before.match(HASH_RE);
    if (!m) { setHashMenu(null); return; }
    setHashSource("post");
    const pos = caretXY(ta);
    setHashMenu({ query: m[1], x: pos.x, y: pos.y });
    setHashIndex(0);
  };
  ...
  const insertTag = (tagStr) => {
    if (hashSource === "post") { insertTagPost(tagStr); return; }
    removeHashToken();
  ...
  // Replace the active #token in the post textarea with the chosen tag.
  const insertTagPost = (tagStr) => {
    const ta = postRef.current;
    if (!ta) { setHashMenu(null); return; }
    ...
    const m = before.match(HASH_RE);
    if (!m) { setHashMenu(null); return; }
    const tokenStart = caret - m[0].length + m[0].indexOf("#");
    const next = ta.value.slice(0, tokenStart) + "#" + tagStr + " " + ta.value.slice(caret);
    setPostBody(next.slice(0, POST_LIMIT));
    setHashMenu(null);
    const newCaret = tokenStart + tagStr.length + 2;
    requestAnimationFrame(() => {
      if (postRef.current) { postRef.current.focus(); postRef.current.setSelectionRange(newCaret, newCaret); }
    });
  };
  ...
  const selectHashItem = (item) => {
    if (!item) return;
    if (item.type === "forum") {
      if (hashSource === "post") insertTagPost(cleanTag(item.forum.name));
      else pickForum(item.forum);
    }
  ...
  // Same menu keyboard nav as the letter editor, for the post textarea.
  ...
  // The # autocomplete dropdown, shared by the letter editor and the post
  // textarea. Built here (before the post early-return) so BOTH composer
  // branches can render it — the post branch is a separate return subtree.
  const hashMenuEl = (hashMenu && getHashItems().length > 0) ? (
        ...
        <div style={{ flex:"1.4 1 560px", minWidth:0, display:"flex", flexDirection:"column", gap:20 }}>
          ...
          <div style={{ background:"#fff", border:"1px solid #E8E0D0", borderRadius:10, padding:"16px 18px" }}>
            <textarea
              ref={postRef}
              value={postBody}
              onChange={e => { setPostBody(e.target.value.slice(0, POST_LIMIT)); detectPostHashMenu(); }}
              onKeyUp={e => { if (!["ArrowDown","ArrowUp","Enter","Tab","Escape"].includes(e.key)) detectPostHashMenu(); }}
              onClick={detectPostHashMenu}
          ...
          <div style={{ display:"flex", gap:10 }}>
            ...
            <div style={{ flex:1, minWidth:0, background:"#fff", border:"1px solid #E8E0D0", borderRadius:10, overflow:"hidden" }}>
              <div style={{ padding:"14px 20px 0" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", paddingBottom:12, borderBottom:"1px solid #F0EDE8", marginBottom:18 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <div style={{ width:28, height:28, borderRadius:2, display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Playfair Display', serif", fontWeight:900, fontSize:13,
                      background: publication ? "#141414" : "none", color: publication ? "#C8A96E" : "#B0A488", border: publication ? "none" : "1px dashed #E8E0D0" }}>
                      {publication ? publication.title.charAt(0).toUpperCase() : "\u270E"}
                    </div>
                    <div>
                      <div style={{ fontFamily:"'DM Sans', sans-serif", fontSize:13, fontWeight:600 }}>
                        {publication ? `${publication.title} · Issue ${(publication.issue_count || 0) + 1}` : "A blank page"}
                      </div>
                      <div style={{ fontFamily:"'DM Mono', monospace", fontSize:9, color:"#B0A488" }}>
                        {publication
                          ? `Goes to ${publication.follower_count || 0} ${publication.follower_count === 1 ? "reader" : "readers"} on publish`
                          : "Lands in the feed and on your page"}
                      </div>
                    </div>
                  </div>
                  <button onClick={() => navigate("/write")}
...
function AudioProvider({ session, children }) {
  ...
  const play = async (episode, show) => {
    ...
    supabase.rpc("mark_read", { p_kind: "episode", p_ref: episode.id }).then(() => {}, () => {});
...
function PlayerBar() {
  ...
  return (
    <div className="letters-playerbar" onClick={() => setExpanded(true)}
      style={{ position:"fixed", left:"var(--pb-left, 0px)", right:"var(--pb-right, 0px)", bottom:"var(--pb-bottom, 0px)", zIndex:45, background:"rgba(20,18,14,0.97)", backdropFilter:"blur(12px)", cursor:"pointer", boxSizing:"border-box" }}>
...
// ============================================================================
// Daily Briefing — a compact strip pinned to the TOP of the Read tab. Shows the
// day's picks from the user's saved sources (publications, podcasts, topics),
// each dismissable (✕) with a "clear all" for the whole strip. Dismiss-state is
// per-day and stored in localStorage (ephemeral, resets at date rollover, no DB
// round-trip; per-device is fine for a daily glance). An "Edit" affordance flips
// the strip into the source picker. Reused later on the You page.
//
// Assumes module-level helpers from App.jsx: supabase, useNavigate, timeAgoRead,
// stripHtmlText, ShowArt.
// ============================================================================

function BriefingStrip({ session }) {
  const navigate = useNavigate();
  const me = session?.user?.id;
  const today = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
  const dismissKey = me ? `letters_briefing_dismissed_${me}_${today}` : null;

  const [loading, setLoading] = useState(true);
  const [prefs, setPrefs] = useState(null);
  const [mode, setMode] = useState("strip");       // "strip" | "setup"
  const [packet, setPacket] = useState({ episodes: [], letters: [], articles: [] });

  // dismiss state for today: { all:bool, items:[itemKey,...] }
  const [dismissed, setDismissed] = useState({ all: false, items: [] });

  // catalogs + selection for setup
  ...
  // read today's dismiss-state from localStorage on mount
  useEffect(() => {
  ...
  const persistDismissed = (next) => {
    setDismissed(next);
    try { if (dismissKey) window.localStorage.setItem(dismissKey, JSON.stringify(next)); } catch { /* ignore */ }
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: pref }, { data: pubs }, { data: shows }, { data: topics }] = await Promise.all([
        ...
        supabase.from("publications").select("id, title, slug").eq("archived", false).order("title"),
        supabase.from("podcast_shows").select("id, title, author, image_url").order("title"),
        supabase.from("topics").select("tag").order("tag"),
      ]);
      if (cancelled) return;
      setAllPubs(pubs || []); setAllShows(shows || []); setAllTopics((topics || []).map(t => t.tag));
      if (pref) {
        setPrefs(pref);
  ...
  // build the packet when prefs present
  useEffect(() => {
    if (!prefs) return;
    let cancelled = false;
    (async () => {
      ...
      const [ep, lt, ar] = await Promise.all([
        showIds.length ? supabase.from("podcast_episodes").select("id, title, published_at, image_url, show_id, show:show_id (id, title, image_url)").in("show_id", showIds).order("published_at", { ascending: false }).limit(6) : Promise.resolve({ data: [] }),
        pubIds.length ? supabase.from("letters").select("id, title, body, created_at, publication_id, profiles:user_id (username, full_name)").in("publication_id", pubIds).order("created_at", { ascending: false }).limit(6) : Promise.resolve({ data: [] }),
        topics.length ? supabase.from("news_articles").select("id, title, source, published_at, category").in("category", topics).order("published_at", { ascending: false }).limit(6) : Promise.resolve({ data: [] }),
      ]);
      if (cancelled) return;
  ...
  const isDismissed = (key) => dismissed.items.includes(key);
  const dismissItem = (key) => persistDismissed({ ...dismissed, items: [...dismissed.items, key] });
  const clearAll = () => persistDismissed({ all: true, items: dismissed.items });
  ...
  const save = async () => {
    if (!me) return;
    setSaving(true);
    const { data, error } = await supabase.from("briefing_preferences").upsert({
      user_id: me, publication_ids: selPubs, podcast_show_ids: selShows, topics: selTopics,
      delivery_time: deliveryTime.length === 5 ? deliveryTime + ":00" : deliveryTime, is_public: isPublic, enabled: true,
    }, { onConflict: "user_id" }).select().single();
    setSaving(false);
    if (!error && data) { setPrefs(data); setMode("strip"); }
  };

  // ---- assemble visible (non-dismissed) items across all three types ----
  const items = [];
  packet.episodes.forEach(ep => items.push({ key: "ep_" + ep.id, kind: "Podcast", title: ep.title, sub: (ep.show?.title || "") + (ep.published_at ? " · " + timeAgoRead(ep.published_at) : ""), img: ep.image_url || ep.show?.image_url, go: () => navigate("/listen") }));
  packet.letters.forEach(l => items.push({ key: "lt_" + l.id, kind: "Newsletter", title: l.title || stripHtmlText(l.body).slice(0, 60), sub: (l.profiles?.full_name || l.profiles?.username || "") + (l.created_at ? " · " + timeAgoRead(l.created_at) : ""), go: () => navigate("/feed/letter/" + l.id) }));
  packet.articles.forEach(a => items.push({ key: "ar_" + a.id, kind: "Topic · " + (a.category || ""), title: a.title, sub: (a.source || "") + (a.published_at ? " · " + timeAgoRead(a.published_at) : ""), go: () => navigate("/read/article/" + a.id) }));
  const visible = items.filter(it => !isDismissed(it.key));

  const hasSources = (prefs?.publication_ids?.length || 0) + (prefs?.podcast_show_ids?.length || 0) + (prefs?.topics?.length || 0) > 0;

  // styles
  const wrap = { background: "#fff", border: "1px solid #E8E0D0", borderRadius: 12, padding: "15px 18px", marginBottom: 18 };
  const kicker = { fontFamily: "'DM Mono', monospace", fontSize: 10.5, letterSpacing: "0.12em", textTransform: "uppercase", color: "#C8A96E" };
  const label = { fontFamily: "'DM Mono', monospace", fontSize: 11, letterSpacing: "0.14em", textTransform: "uppercase", color: "#C8A96E", marginBottom: 8 };
  const chip = (on) => ({ display: "inline-flex", alignItems: "center", gap: 5, padding: "6px 12px", borderRadius: 18, border: on ? "1px solid #141414" : "1px solid #E2DAC9", background: on ? "#141414" : "#fff", color: on ? "#F0EAD8" : "#555", fontFamily: "'DM Sans', sans-serif", fontSize: 12.5, cursor: "pointer", marginRight: 7, marginBottom: 7 });
  const searchBox = { width: "100%", boxSizing: "border-box", border: "1px solid #E2DAC9", borderRadius: 9, padding: "7px 11px", fontSize: 13, fontFamily: "'DM Sans', sans-serif", marginBottom: 9 };

  // ---------- render ----------
  if (loading) return null;                 // don't flash an empty box while loading
  if (!me) return null;

  // Not set up yet → a slim invitation, unless already cleared for today.
  if (!prefs || !hasSources) {
    if (mode !== "setup") {
      return (
        <div style={{ ...wrap, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
          <div>
            <div style={kicker}>Daily Briefing</div>
            <div style={{ fontFamily: "'EB Garamond', serif", fontSize: 15, color: "#444", marginTop: 3 }}>Gather your newsletters, podcasts, and topics into one daily read.</div>
          </div>
          <button onClick={() => setMode("setup")} style={{ flexShrink: 0, background: "#141414", color: "#F0EAD8", border: "none", borderRadius: 20, padding: "9px 18px", fontFamily: "'DM Sans', sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Set it up</button>
        </div>
      );
    }
  }

  // Setup mode (shared by first-run and Edit)
  if (mode === "setup") {
    ...
    return (
      <div style={wrap}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
          <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 19, fontWeight: 800 }}>Your Daily Briefing<span style={{ color: "#C8A96E" }}>.</span></div>
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {pubMatches.length === 0 ? <p style={{ fontFamily: "'EB Garamond', serif", fontStyle: "italic", fontSize: 13.5, color: "#999" }}>None yet — as writers start publications on Letters, they'll appear here.</p>
            : pubMatches.slice(0, 24).map(p => <button key={p.id} onClick={() => togglePub(p.id)} style={chip(selPubs.includes(p.id))}>{selPubs.includes(p.id) ? "✓ " : ""}{p.title}</button>)}
        </div>

        <div style={{ marginBottom: 16 }}>
          <div style={label}>Podcasts</div>
          <input value={showSearch} onChange={e => setShowSearch(e.target.value)} placeholder="Search podcasts…" style={searchBox}/>
          {showMatches.length === 0 ? <p style={{ fontFamily: "'EB Garamond', serif", fontStyle: "italic", fontSize: 13.5, color: "#999" }}>None yet — add podcasts from the Listen tab.</p>
            : showMatches.slice(0, 24).map(sh => <button key={sh.id} onClick={() => toggleShow(sh.id)} style={chip(selShows.includes(sh.id))}>{selShows.includes(sh.id) ? "✓ " : ""}{sh.title}</button>)}
        </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          {topicMatches.length === 0 ? <p style={{ fontFamily: "'EB Garamond', serif", fontStyle: "italic", fontSize: 13.5, color: "#999" }}>None yet.</p>
            : topicMatches.slice(0, 30).map(t => <button key={t} onClick={() => toggleTopic(t)} style={chip(selTopics.includes(t))}>{selTopics.includes(t) ? "✓ " : ""}#{t}</button>)}
        </div>

        <div style={{ borderTop: "1px solid #EDE6D8", paddingTop: 14, marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 11, flexWrap: "wrap" }}>
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13.5, color: "#333" }}>Email it each day at</span>
            <input type="time" value={deliveryTime} onChange={e => setDeliveryTime(e.target.value)} style={{ border: "1px solid #E2DAC9", borderRadius: 8, padding: "5px 9px", fontFamily: "'DM Mono', monospace", fontSize: 12.5 }}/>
          </div>
          <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
            <input type="checkbox" checked={isPublic} onChange={e => setIsPublic(e.target.checked)} style={{ width: 15, height: 15 }}/>
            <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13.5, color: "#333" }}>Show “What I'm Reading” on my profile</span>
          </label>
        </div>

        <div style={{ display: "flex", gap: 9 }}>
          <button onClick={save} disabled={saving} style={{ background: "#141414", color: "#F0EAD8", border: "none", borderRadius: 20, padding: "9px 22px", fontFamily: "'DM Sans', sans-serif", fontSize: 13.5, fontWeight: 600, cursor: saving ? "wait" : "pointer" }}>{saving ? "Saving…" : "Save"}</button>
          {prefs && <button onClick={() => setMode("strip")} style={{ background: "none", color: "#888", border: "1px solid #E2DAC9", borderRadius: 20, padding: "9px 16px", fontFamily: "'DM Sans', sans-serif", fontSize: 13.5, cursor: "pointer" }}>Cancel</button>}
        </div>
      </div>
    );
  }

  // Strip cleared for the day, or every item individually cleared → show nothing.
  if (dismissed.all || visible.length === 0) return null;

  // The strip.
  return (
    <div style={wrap}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 17, fontWeight: 800 }}>Your Daily Briefing<span style={{ color: "#C8A96E" }}>.</span></div>
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <button onClick={() => setMode("setup")} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", fontSize: 12.5, color: "#888" }}>Edit</button>
          <button onClick={clearAll} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "'EB Garamond', serif", fontStyle: "italic", fontSize: 13, color: "#B0A488" }}>clear all</button>
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "9px 18px" }} className="briefing-mini">
        {visible.map(it => (
          <div key={it.key} style={{ display: "flex", gap: 9, alignItems: "flex-start", borderLeft: "2px solid #C8A96E", paddingLeft: 9 }}>
            <div onClick={it.go} style={{ flex: 1, minWidth: 0, cursor: "pointer" }}>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 8.5, letterSpacing: "0.1em", textTransform: "uppercase", color: "#C8A96E" }}>{it.kind}</div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 13.5, fontWeight: 700, lineHeight: 1.2, color: "#141414", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{it.title}</div>
              <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: "#B0A488", marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{it.sub}</div>
            </div>
            <button onClick={() => dismissItem(it.key)} title="Mark as read" style={{ flexShrink: 0, background: "none", border: "none", cursor: "pointer", color: "#C9BFA9", fontSize: 15, lineHeight: 1, padding: "0 2px" }}>✕</button>
          </div>
        ))}
      </div>
    </div>
      ...
      <Routes>
        ...
        <Route path="/signin" element={
          <div style={{ minHeight:"100vh", background:"#F9F6F0", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20 }}>
            ...
            <Auth onAuthSuccess={onAuthSuccess} navigate={goTo}/>
          </div>
        }/>
...
function MobileChrome() {
  useEffect(() => {
    ...
    const apply = () => {
      // Keyboard height = layout viewport - visual viewport. Do NOT subtract
      // vv.offsetTop: that is the page's scroll position, not part of the
      // keyboard, and including it made --kb both wrong and scroll-dependent.
      const covered = Math.max(0, window.innerHeight - vv.height);
      root.style.setProperty("--kb", `${Math.round(covered)}px`);
      // Publish the visible height too, so fixed overlays can size themselves
      // against what is actually on screen rather than guessing.
  ...
  return (
    <style>{`
      :root { --kb: 0px; --vvh: 100vh; --safe-b: env(safe-area-inset-bottom, 0px); --safe-t: env(safe-area-inset-top, 0px); }
      ...
      /* ROOT CAUSE of the "view jumps left" bug: iOS auto-zooms the page when a
         focused input has font-size < 16px, then pans to keep the caret in view.
         It is NOT a layout overflow — the document is the correct width and is
         simply being magnified (visualViewport.scale was 1.14). React writes
         fontSize inline, which beats a plain stylesheet rule, so this needs
         !important to win. */
      ...
      /* The player bar's position comes from these vars, read by its inline style
         — inline beats stylesheets, so vars are the only way CSS can steer it.
         Mobile: flush on top of the real, measured nav height (a hardcoded 64px
         left a floating gap). Desktop: inset between the fixed side navs
         (220px / 160px), which sit at a higher z-index and were covering it. */
      :root { --pb-left: 0px; --pb-right: 0px; --pb-bottom: var(--nav-h, 64px); }
      @media (min-width: 768px) {
        :root { --pb-left: 220px; --pb-right: 160px; --pb-bottom: 0px; }
      }
      ...
      /* The reader overlay is fixed with bottom:0 — but bottom:0 is the LAYOUT
         viewport, which iOS does not shrink when the keyboard opens. The result
         is an uncovered band at the bottom where the feed shows through. Pull
         the overlay's floor up by the keyboard height so it always covers what
         is actually visible. */
      .desk-cols { display: grid; grid-template-columns: 1fr !important; gap: 10px !important; }
      @media (min-width: 820px) { .desk-cols { grid-template-columns: 1.35fr 1fr !important; gap: 32px !important; } }
      .desk-read .desk-cta { opacity: 0; transition: opacity .14s; }
      .desk-read:hover .desk-cta { opacity: 1; }
